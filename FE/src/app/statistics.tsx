...
import { format, subDays, parseISO, startOfWeek, endOfWeek, eachDayOfInterval, isSameDay, startOfMonth, endOfMonth, eachWeekOfInterval, isSameWeek, subWeeks, addWeeks, subMonths, addMonths } from 'date-fns';
...
const StatisticsScreen = () => {
  ...
  const statsData = useMemo(() => {
    if (!shares.length || Object.keys(expensesMap).length === 0) return { chart: [], total: 0, list: [], dateRangeLabel: '' };
    ...
    if (viewMode === 'week') {
        ...
        const startLoop = startOfWeek(monthStart, { locale: vi, weekStartsOn: 1 });
        ...
        chartData = weeks.map((weekStart, index) => {
            ...
            if (actualStart > actualEnd) return { label: `W${index + 1}`, value: 0 };
    ...
    const list = filteredShares.map(s => ({
        ...s,
    ...
    <View style={styles.container}>
      ...
      <View style={styles.tabContainer}>
        <TouchableOpacity 
            ...
            onPress={() => { setViewMode('week'); setCurrentDate(new Date()); }}
        >
            <Text style={[styles.tabText, viewMode === 'week' && styles.activeTabText]}>Tuần</Text>
        </TouchableOpacity>
        <TouchableOpacity 
            style={[styles.tab, viewMode === 'month' && styles.activeTab]}
            onPress={() => { setViewMode('month'); setCurrentDate(new Date()); }}
        >
            <Text style={[styles.tabText, viewMode === 'month' && styles.activeTabText]}>Tháng</Text>
        </TouchableOpacity>
      </View>
            ...
            <View style={styles.chartContainer}>
                ...
                <View style={{flexDirection: 'row', height: 200}}>
                    ...
                    <View style={styles.yAxis}>
                        <Text style={styles.axisLabel}>{formatYLabel(maxChartValue)}</Text>
                        <Text style={styles.axisLabel}>{formatYLabel(maxChartValue * 0.66)}</Text>
                        <Text style={styles.axisLabel}>{formatYLabel(maxChartValue * 0.33)}</Text>
                    ...
                    <View style={styles.chart}>
                        {statsData.chart.map((item, index) => (
                            ...
                            >
                                    ...
                                    <View 
                                        style={[
                                            ...
                                            { 
                                                ...
                                                backgroundColor: selectedPoint?.label === item.label ? APP_COLOR.ORANGE : '#FFCCBC'
                                            }
                                        ]} 
                                    />
                                </View>
                                <Text style={[
                                    styles.barLabel,
                                    selectedPoint?.label === item.label && styles.selectedLabel
                                ]}>
                                    {item.label}
                                </Text>
                            </TouchableOpacity>
                    ...
                    statsData.list.map((item) => (
                        <View key={item.id} style={styles.item}>
                            <View style={styles.itemIcon}>
                                <Ionicons name="receipt-outline" size={24} color={APP_COLOR.ORANGE} />
                            </View>
                            <View style={styles.itemInfo}>
                                <Text style={styles.itemTitle}>{item.expense.description}</Text>
                                <Text style={styles.itemDate}>
                                    {format(parseISO(item.expense.createdTime), 'dd/MM/yyyy HH:mm')}
                                </Text>
                            </View>
