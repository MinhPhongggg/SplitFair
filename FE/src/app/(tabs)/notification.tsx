...
import { useGetNotifications, useMarkAsRead, useMarkAllAsRead, useGetGroupById, useGetExpenseById } from '@/api/hooks';
    ...
    try {
        ...
        if (item.type === 'GROUP_INVITE' && item.referenceId) {
            ...
            try {
                ...
                router.push({ pathname: '/(tabs)/groups/[groupId]', params: { groupId: item.referenceId } });
            } catch (e) {
            ...
            try {
                ...
                router.push({ pathname: '/(tabs)/groups/expense/[expenseId]', params: { expenseId: item.referenceId } });
            } catch (e) {
            ...
            try {
                ...
                router.push({ pathname: '/(tabs)/groups/[groupId]', params: { groupId: item.referenceId } });
                 ...
                 Alert.alert("Thông báo", "Nhóm liên quan không còn tồn tại hoặc đã bị xóa.");
            }
        }
    } catch (error) {
      ...
      >
        ...
        <View style={styles.contentBox}>
          ...
          <Text style={styles.time}>
            {item.createdTime ? formatDistanceToNow(new Date(item.createdTime), { addSuffix: true, locale: vi }) : ''}
          </Text>
        </View>
  ...
  if (isError || (!isLoading && !isValidData && notifications)) {
      return (
          <View style={styles.center}>
              ...
              <Text style={{marginTop: 10, color: 'red'}}>Không thể tải thông báo.</Text>
              <TouchableOpacity onPress={() => refetch()} style={{marginTop: 20, padding: 10, backgroundColor: '#eee', borderRadius: 8}}>
                  <Text>Thử lại</Text>
              </TouchableOpacity>
          </View>
      );
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <FlatList
        ...
        refreshControl={
          <RefreshControl refreshing={isLoading} onRefresh={refetch} colors={[APP_COLOR.ORANGE]} />
        }
        ListEmptyComponent={
