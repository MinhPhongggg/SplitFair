...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, ActivityIndicator, RefreshControl, Alert, LayoutAnimation, Platform, UIManager, ScrollView, TextInput } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { router } from 'expo-router';
import { useCurrentApp } from '@/context/app.context';
import { useGetAllDebtsByUser, useSettleDebt, useSettleBatchDebts } from '@/api/hooks';
...
const DebtScreen = () => {
  ...
  const { groupedPayables, groupedReceivables, overview, suggestions } = useMemo(() => {
    if (!allDebts) return { groupedPayables: [], groupedReceivables: [], overview: { pay: 0, rec: 0, net: 0 }, suggestions: [] };
    ...
    const groupData = (items: Debt[], isPayable: boolean): GroupedDebt[] => {
        ...
        items.forEach(item => {
            ...
            const partnerName = rawName || `User ${partnerId?.substring(0,5) ?? 'Unknown'}`;
            const partnerAvatar = isPayable ? item.toUserAvatar : item.fromUserAvatar;

            // Filter by search query
            if (searchQuery && !partnerName.toLowerCase().includes(searchQuery.toLowerCase())) {
                return;
            }
      ...
      <View style={styles.overviewContainer}>
          <View style={styles.overviewCard}>
              ...
              <View style={styles.netBalanceSection}>
                  ...
                  <Text style={[styles.netValueBig, overview.net >= 0 ? styles.textGreen : styles.textRed]}>
                      {overview.net > 0 ? '+' : ''}{overview.net.toLocaleString('vi-VN')} đ
                  </Text>
              </View>
              ...
              <View style={styles.overviewRow}>
                  <View style={styles.overviewItem}>
                      <Text style={[styles.overviewValue, styles.textRed]}>{overview.pay.toLocaleString('vi-VN')} đ</Text>
                  ...
                  {suggestions.map((s, index) => (
                      <View key={index} style={styles.suggestionCard}>
                          <View style={styles.suggestionIcon}>
                              <Ionicons name="swap-horizontal" size={24} color={APP_COLOR.ORANGE} />
                          </View>
                          <View style={styles.suggestionContent}>
                              <Text style={styles.suggestionText}>
                                  Nợ chéo với <Text style={{fontWeight: 'bold'}}>{s.partnerName}</Text>
                              </Text>
                              <Text style={styles.suggestionDetail}>
                                  Thay vì trả {s.payAmount.toLocaleString()}đ & nhận {s.recAmount.toLocaleString()}đ
                              </Text>
                              <View style={styles.suggestionActionContainer}>
                                  <Text style={[styles.suggestionAction, s.action === 'RECEIVE' ? styles.textGreen : styles.textRed]}>
                                      {s.action === 'RECEIVE' ? 'NHẬN' : 'TRẢ'} {s.netAmount.toLocaleString()} đ
                                  </Text>
                              </View>
                          </View>
                      </View>
  ...
  const renderGroupItem = ({ item }: { item: GroupedDebt }) => {
    ...
    const percentage = totalGlobal > 0 ? (item.totalAmount / totalGlobal) * 100 : 0;
            ...
            <View style={styles.headerLeft}>
                ...
                <View style={styles.userInfo}>
                    ...
                    <View style={styles.progressBarContainer}>
                        <View style={[styles.progressBarFill, { width: `${percentage}%`, backgroundColor: isPayable ? '#FF453A' : '#32D74B' }]} />
                    </View>
                    <Text style={styles.debtCount}>{item.debts.length} khoản nợ • {percentage.toFixed(1)}% tổng nợ</Text>
                </View>
            </View>
            <View style={styles.headerRight}>
                <Text style={[styles.totalAmount, isPayable ? styles.textRed : styles.textGreen]}>
        ...
        {isExpanded && (
            <View style={styles.debtList}>
                <View style={styles.actionRow}>
                    <TouchableOpacity style={styles.settleAllBtn} onPress={() => handleSettleAll(item.partnerId, item.debts)}>
                        <Text style={styles.settleAllText}>Thanh toán tất cả ({item.debts.length})</Text>
                    </TouchableOpacity>
                </View>
                            ...
                            >
                                <View style={styles.debtInfo}>
                                    ...
                                    <Text style={styles.debtDate}>
                                        {debt.createdTime 
                                            ? format(new Date(debt.createdTime), 'dd/MM/yyyy HH:mm', { locale: vi })
                                            : 'Không có ngày'}
                                    </Text>
                                </View>
                                <View style={styles.debtAction}>
                                    <Text style={styles.debtAmount}>{debt.amount.toLocaleString('vi-VN')} đ</Text>
                                    <View style={{flexDirection: 'row'}}>
                                        {!isPayable && (
                                            <TouchableOpacity style={[styles.iconBtn, {marginRight: 8}]} onPress={() => showToast('info', 'Nhắc nợ', `Đã gửi thông báo đến ${item.partnerName}`)}>
                                                <Ionicons name="notifications-outline" size={18} color={APP_COLOR.ORANGE} />
                                            </TouchableOpacity>
                                        )}
                                        <TouchableOpacity 
                                            style={[styles.settleBtn, isPayable ? styles.btnOutline : styles.btnFill]}
                                            onPress={() => handleSettle(debt)}
                                        >
                                            <Text style={[styles.settleBtnText, !isPayable && {color: 'white'}]}>
                                                {isPayable ? "Trả" : "Nhận"}
                                            </Text>
                                        </TouchableOpacity>
                                    </View>
  ...
  const currentData = activeTab === 'PAYABLES' ? groupedPayables : groupedReceivables;
...
const styles = StyleSheet.create({
  ...
  overviewCard: {
    backgroundColor: 'white', borderRadius: 16, padding: 0, // Remove padding to handle dividers
  ...
  suggestionActionContainer: { alignSelf: 'flex-start', backgroundColor: '#F5F7FA', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6 },
  suggestionAction: { fontSize: 13, fontWeight: 'bold' },

  tabsContainer: { flexDirection: 'row', marginHorizontal: 15, marginBottom: 10, backgroundColor: 'white', borderRadius: 12, padding: 4 },
  ...
  // Group Card Styles
  groupCard: {
    backgroundColor: 'white', borderRadius: 12, marginHorizontal: 15, marginBottom: 12, overflow: 'hidden',
    shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, elevation: 2
  },
  groupHeader: {
  ...
  progressBarContainer: { height: 4, backgroundColor: '#eee', borderRadius: 2, marginTop: 4, width: '80%' },
  progressBarFill: { height: '100%', borderRadius: 2 },

  // Debt List Styles
  debtList: { backgroundColor: '#FAFAFA', borderTopWidth: 1, borderTopColor: '#eee', paddingBottom: 10 },
  actionRow: { padding: 10, alignItems: 'center' },
  settleAllBtn: { paddingVertical: 8, paddingHorizontal: 20, backgroundColor: '#E3F2FD', borderRadius: 20 },
  settleAllText: { color: '#2196F3', fontWeight: '600', fontSize: 13 },

  subGroupContainer: { marginTop: 10 },
  subGroupTitle: { fontSize: 13, fontWeight: 'bold', color: '#666', marginLeft: 15, marginBottom: 5, textTransform: 'uppercase' },
  
  debtItem: { 
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', 
    padding: 12, paddingHorizontal: 15, borderBottomWidth: 1, borderBottomColor: '#f0f0f0', backgroundColor: 'white'
  },
  debtInfo: { flex: 1 },
  ...
  debtAmount: { fontSize: 14, fontWeight: '600', color: '#333', marginBottom: 6 },
  
  settleBtn: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 6, borderWidth: 1 },
  iconBtn: { padding: 6 },
