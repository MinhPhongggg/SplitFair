...
interface Props { item: any; onPress: () => void; getAvatar: (id: string) => string | undefined; }
      ...
      <View style={styles.suggestionRow}>
        <View style={{ width: 60, height: 36, marginRight: 5 }}>
            <View style={{ position: 'absolute', left: 0, zIndex: 2 }}>
                <Avatar name={item.from} avatar={getAvatar(item.fromId)} size={36} style={{ marginRight: 0 }} />
            </View>
            <View style={{ position: 'absolute', left: 20, zIndex: 1 }}>
                <View style={{ borderWidth: 2, borderColor: 'white', borderRadius: 18 }}>
                    <Avatar name={item.to} avatar={getAvatar(item.toId)} size={36} style={{ marginRight: 0 }} />
                </View>
            </View>
        </View>
...
const styles = StyleSheet.create({
  suggestionItem: { paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#f5f5f5' },
  suggestionRow: { flexDirection: 'row', alignItems: 'center' },
  suggestionAmount: { fontSize: 15, fontWeight: 'bold', color: '#FF3B30', marginLeft: 10 },
