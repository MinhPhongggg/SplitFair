// src/app/(tabs)/groups/expense/edit/[expenseId].tsx
...
const EditExpenseScreen = () => {
      ...
      },
      {
        onSuccess: () => {
          ...
          saveShares(shareRequest, {
            onSuccess: () => {
              showToast('success', 'Thành công', 'Đã cập nhật chi tiêu và phần chia.');
              router.back();
            },
            onError: (err) => {
              showToast('error', 'Lỗi lưu phần chia', err.message);
            },
          });
        },
        onError: (err) => {
          showToast('error', 'Lỗi cập nhật chi tiêu', err.message);
        },
      }
    );
  };
  ...
  if (isLoadingExpense || isLoadingMembers || isLoadingShares) {
    return (
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Chỉnh sửa chi tiêu</Text>
        <View style={{ width: 34 }} /> 
      </View>
